...
window.renderStoryPage = function(){
  ...
  if(ids.length){
    STORY_MINI = window.buildMiniSlideshow(viewer.querySelector('#storySlideWrap'), ids, storyDurationMs());
  }
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  const next = $('storyNext'); if(next) next.onclick = ()=>{ storyNext(); if(STORY_AUTOPLAY){ startStoryAutoPlay(); } };
  const prev = $('storyPrev'); if(prev) prev.onclick = ()=>{ storyPrev(); if(STORY_AUTOPLAY){ startStoryAutoPlay(); } };
  const close = $('storyClose'); if(close) close.onclick = closeStoryModal;
  const pause = $('storySlidePause');
  if(pause) pause.onclick = ()=>{
    if(STORY_T){ clearTimeout(STORY_T); clearInterval(STORY_T); STORY_T = null; pause.textContent = '▶'; }
    else { startStoryAutoPlay(); pause.textContent = '⏸'; }
  };
  const modal = $('storyModal');
  if(modal) modal.addEventListener('click', (e)=>{ if(e.target===modal) closeStoryModal(); });
});
