/* ============================================================
   ...
   VERSION 2 — Safari/Chrome mobile crash fix
   ------------------------------------------------------------
   Key fixes vs older version:
   • NO forced reflow (void img.offsetWidth) on slides
   • NO animation reset (style.animation='none') on img/video
   • Video preload = 'metadata' (not 'auto')
   • Video play delayed 200ms so DOM/transition settles
   • Effect classes added ONLY, never re-triggered
   • Image lazy-loads only current + next 2 slides
   • Photos preloaded before timer starts (so no mid-slide reload)
   • Token-based timers — no double-fire after navigation
   • Music volume duck/restore only — never touches src
   ============================================================ */
(function(){
'use strict';
...
let SS = [], SS_IDX = 0, SS_T = null, SS_VIDEO_TIMER = null;
let SS_isOpen = false;
...
function SS_resetVideo(v){
  if(!v) return;
  try{ v.pause(); v.currentTime = 0; v.muted = true; v.volume = 0; }catch(e){}
}
...
/* ---------- Volume-only fade (never touches src/play) ---------- */
...
function SS_normalMusicVol(){ return window.getVol ? window.getVol('slideshow') : 0.85; }
function SS_duckedMusicVol(){ return Math.max(0.05, SS_normalMusicVol()*0.35); }

/* ---------- Start slideshow music (once) ---------- */
function SS_startMusicOnce(){
  const a = $('audioPlayer'); if(!a) return;
  const mode = (S.CURR.shared||{}).music_mode || 'both';
  if(mode==='card') return;

  const ssList = window.buildPlaylistFor ? window.buildPlaylistFor('slideshow') : [];
  if(!ssList || !ssList.length){
    if(window.__setCurrCtx__) window.__setCurrCtx__('slideshow', []);
    if(a.src && !a.paused && mode==='slideshow') a.pause();
    return;
  }
...
/* ---------- Lazy image loading (Safari-safe) ---------- */
function SS_loadSlideImage(idx){
  ...
  /* Smaller width on phones = less memory = no crash */
  const w = window.innerWidth < 700 ? 1200 : 2000;
  img.src = 'https://lh3.googleusercontent.com/d/'+s.drive_id+'=w'+w;
  img.onerror = ()=>{
    img.src = 'https://drive.google.com/thumbnail?id='+s.drive_id+'&sz=w'+w;
  };
}
function SS_preloadAhead(){
  SS_loadSlideImage(SS_IDX);
  if(SS.length>1) SS_loadSlideImage((SS_IDX+1)%SS.length);
  if(SS.length>2) SS_loadSlideImage((SS_IDX+2)%SS.length);
  /* Release far-away images to keep memory low on phones */
  const t = $('slidesTrack'); if(!t) return;
...
function SS_buildSlides(){
  ...
  SS.forEach((s,i)=>{
    ...
    if(s.type==='video'){
      ...
      v.preload = 'metadata';            /* <-- KEY: was 'auto', now 'metadata' */
      ...
      img.loading = 'lazy';
      img.dataset.loaded = '0';
      /* src is set lazily by SS_loadSlideImage */
      d.appendChild(img);
    }
...
/* -------------------------------------------------------------
   SAFE effect applier.
   NO forced reflow (void offsetWidth).
   NO style.animation reset.
   Just adds a class — CSS animation plays automatically.
   ------------------------------------------------------------- */
function SS_applyEffectToCurrent(){
  ...
  if(cur.type==='video'){
    slideEl.classList.add(SS_pickVideoEffect());
  } else {
    slideEl.classList.add(SS_pickPhotoEffect());
...
function SS_updateSlide(){
  ...
  if(cur.type==='video'){
    if(SS_musicDucked!==true){
      SS_fadeVolume(SS_duckedMusicVol(), 300);
      SS_musicDucked = true;
    }
    const v = slideEl.querySelector('video');
    const ub = slideEl.querySelector('.unmute-btn');
    if(v){
      /* Delay play by 200ms — lets DOM/transition settle.
         This was the biggest cause of Safari crash. */
    ...
    if(SS_musicDucked!==false){
      SS_fadeVolume(SS_normalMusicVol(), 300);
    ...
    if(img && img.dataset.loaded!=='1'){
      /* Image not loaded yet → wait for it, don't count the timer */
      img.addEventListener('load', startTimer, {once:true});
      img.addEventListener('error', startTimer, {once:true});
      setTimeout(startTimer, 6000);   /* safety fallback */
...
function SS_close(){
  ...
  if(t){
    Array.from(t.children).forEach(slideEl=>{
      ...
      if(ub) ub.classList.remove('show');
      const img = slideEl.querySelector('img.fx-target');
      if(img){ img.removeAttribute('src'); img.dataset.loaded='0'; }
    });
    t.innerHTML = '';
  }
  const dots = $('slideshowDots'); if(dots) dots.innerHTML = '';
  ...
  const a = $('audioPlayer');
  if(a) a.volume = window.getVol ? window.getVol('card') : 0.45;
