/* ============================================================
   admin.js — Admin panel: text, theme, people, lock, extras,
               slideshows, reviews, backup, guests, security
   ============================================================ */
(function(){
'use strict';
...
/* ============================================================
   LOAD / SAVE PERSON INTO ADMIN FIELDS
   ============================================================ */
async function loadPersonIntoAdmin(personId){
  ...
  ADM.shared = shared;
  ADM.theme = shared.theme || 'romantic';
  ...
  ADM.counters = {};
  COUNTERS.forEach(c => {
    ADM.counters[c.labelKey] = shared[c.labelKey] !== undefined ? shared[c.labelKey] : '';
    ADM.counters[c.dtKey]    = shared[c.dtKey] || '';
    ADM.counters[c.tzKey]    = shared[c.tzKey] || DEFAULT_TZ;
    ADM.counters[c.dispKey]  = shared[c.dispKey] || '';
    ADM.counters[c.showKey]  = shared[c.showKey] !== undefined ? shared[c.showKey] : 'true';
  });

  ADM.gifts  = await sb.rows(T_GIFTS,  personId) || [];
  ADM.story  = await sb.rows(T_STORY,  personId) || [];
  ADM.events = await sb.rows(T_EVENTS, personId) || [];
  ADM.voice  = await sb.rows(T_VOICE,  personId) || [];
  ADM.video  = await sb.rows(T_VIDEO,  personId) || [];
  ADM.pins   = await sb.rows(T_PINS,   personId) || [];
  ADM.media  = await sb.rows(T_MEDIA,  personId) || [];
  ...
  renderPinsRepeater();
  renderMediaRepeater();
  renderAdminSlideshowFields(shared);
}
...
function bindAdminActions(){
  ...
  if(previewBtn && previewBtn.dataset._bound !== '1'){
    ...
    previewBtn.onclick = () => saveAdminFull(true, false);
  }
...
async function exportAllBackup(){
  ...
  if(st){ st.textContent = '⏳ Building…'; st.className = 'panel-status'; }
  try{
    if(!window.XLSX) throw new Error('XLSX not loaded');
    const wb = XLSX.utils.book_new();

    const peopleRows = S.PEOPLE.map(p => ({
      id: p.id, slug: p.slug, display_name: p.display_name, birthday: p.birthday,
      password: p.password, enabled: p.enabled, sort_order: p.sort_order,
      requester_name: p.requester_name || '', requester_whatsapp: p.requester_whatsapp || '',
      wipe_iso: p.wipe_iso || ''
    }));
    ...
    for(const p of S.PEOPLE){
      ...
      const rows = Object.keys(set).map(k => ({key: k, value: set[k]}));
      const sheetName = ('set_' + (p.slug || p.id)).slice(0, 31);
    ...
    if(st){ st.textContent = '✅ Exported'; st.className = 'panel-status ok'; }
  }catch(e){
    if(st){ st.textContent = '❌ ' + e.message; st.className = 'panel-status err'; }
  }
}
...
async function importAllBackup(e){
  ...
  if(!f) return;
  const st = $('adminDataStatus');
  if(st){ st.textContent = '⏳ Reading…'; st.className = 'panel-status'; }
  try{
    if(!window.XLSX) throw new Error('XLSX not loaded');
    const buf = await f.arrayBuffer();
    const wb = XLSX.read(buf, {type: 'array'});
    const peopleSheet = wb.Sheets['people'];
    if(!peopleSheet) throw new Error('No "people" sheet');
    ...
    for(const r of rows){
      ...
      if(existing){
        ...
        patch.slug = r.slug;
        await sb.insPerson(patch);
      }
      ok++;
    }
    if(st){ st.textContent = '✅ Imported ' + ok + ' people'; st.className = 'panel-status ok'; }
    S.PEOPLE = await sb.people() || [];
    if(window.buildHome) window.buildHome();
  }catch(err){
...
/* ============================================================
   AUTO-LOAD GUESTS WHEN TAB OPENED
   ============================================================ */
