/* ============================================================
   home.js — Home screen, person grid, admin login trigger
   ============================================================ */
(function(){
'use strict';
...
/* ---------- Admin login trigger (triple-click emoji) ---------- */
  ...
  ep.forEach(p => {
    ...
    b.innerHTML =
      ...
      (bdayLabel
        ...
        : '');
    b.onclick = () => window.onPersonClick(p);
    g.appendChild(b);
  });
...
window.startAdmin = async function(){
  try{
    ...
    else if(window.buildAdminPanel) window.buildAdminPanel();
  }catch(e){
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(aCancel) aCancel.onclick = () => hide($('adminLoginModal'));
  if(aInput) aInput.addEventListener('keydown', e => {
    if(e.key === 'Enter'){ e.preventDefault(); window.tryAdminLogin(); }
  });
  if(aConfirm) aConfirm.onclick = () => window.tryAdminLogin();
